...
homopolymerCorrection <-
    function(sequences, k, kmers, is.rna = FALSE) {
        ...
        mwidthsa <-
            BiocGenerics::width(methods::as(GenomicRanges::gaps(maska),
                                            "Views"))
        viewcnta <-
    ...
    function(foreground.sets,
        ...
        if (length(foreground.sets) > 0) {
            ...
            if (n.cores == 1) {
                ...
                enrichment.dfs <-
                    parallel::parLapply(cl = cluster, foreground.sets,
...
drawVolcanoPlot <-
    function(kmers,
        ...
        part2 <-
            kmers[which(kmers$adj.p.value < significance.threshold), ]
