...
homopolymerCorrection <- function(sequences, k, kmers, is.rna = FALSE) {
  ...
  mwidthsc <- BiocGenerics::width(methods::as(GenomicRanges::gaps(maskc), "Views"))
  viewcntc <- length(methods::as(GenomicRanges::gaps(maskc), "Views"))
  kmers[cMask] <- kmers[cMask] - (sum(mwidthsc) - viewcntc * (k - 1)) + sum(floor(mwidthsc / k))

  gMask <- paste(rep("G", k), collapse = "")
  maskg <- Biostrings::maskMotif(seq, gMask)
  mwidthsg <- BiocGenerics::width(methods::as(GenomicRanges::gaps(maskg), "Views"))
  viewcntg <- length(methods::as(GenomicRanges::gaps(maskg), "Views"))
  kmers[gMask] <- kmers[gMask] - (sum(mwidthsg) - viewcntg * (k - 1)) + sum(floor(mwidthsg / k))

  if(is.rna) {
    uMask <- paste(rep("U", k), collapse = "")
    masku <- Biostrings::maskMotif(seq, uMask)
    mwidthsu <- BiocGenerics::width(methods::as(GenomicRanges::gaps(masku), "Views"))
    viewcntu <- length(methods::as(GenomicRanges::gaps(masku), "Views"))
    kmers[uMask] <- kmers[uMask] - (sum(mwidthsu) - viewcntu * (k - 1)) + sum(floor(mwidthsu / k))
  } else {
    tMask <- paste(rep("T", k), collapse = "")
    maskt <- Biostrings::maskMotif(seq, tMask)
    mwidthst <- BiocGenerics::width(methods::as(GenomicRanges::gaps(maskt), "Views"))
    viewcntt <- length(methods::as(GenomicRanges::gaps(maskt), "Views"))
    kmers[tMask] <- kmers[tMask] - (sum(mwidthst) - viewcntt * (k - 1)) + sum(floor(mwidthst / k))
  }
...
#' @param background.set character vector of DNA or RNA sequences that constitute the
#' background set
...
#' @family \emph{k}-mer functions
#' @export
calculateKmerEnrichment <- function(foreground.sets, background.set, k, permutation = FALSE,
                                    chisq.p.value.threshold = 0.05, p.adjust.method = "BH",
                                    n.cores = 4) {
  if(length(foreground.sets) > 0) {
    ...
    if(n.cores == 1) {
      enrichment.dfs <- lapply(foreground.sets, function(foreground.set) {
        foreground.kmers <- generateKmers(foreground.set, k)
        return(computeKmerEnrichment(foreground.kmers, background.kmers, permutation = permutation,
      ...
      parallel::clusterExport(cl = cluster,
                              varlist = c("k", "background.kmers", "permutation",
                                          "chisq.p.value.threshold", "p.adjust.method",
                                          "computeKmerEnrichment", "generateKmers"),
                              envir = environment())
      enrichment.dfs <- parallel::parLapply(cl = cluster, foreground.sets,
    ...
    return(list(dfs = enrichment.dfs,
                kmers = gsub("T", "U", as.character(names(background.kmers)))))
  } else {
    return(NULL)
  }
}
...
#' rna.sequences <- c("CAACAGCCUUAAUU", "CAGUCAAGACUCC", "CUUUGGGGAAU",
...
#' dna.sequences <- c("CAACAGCCTTAATT", "CAGTCAAGACTCC", "CTTTGGGGAAT",
...
#' RNA sequences are internally converted to DNA sequences. It is not allowed to mix DNA and
#' RNA sequences.
#'
...
generateKmers <- function(sequences, k) {
  kmers <- tryCatch({
    kmer.counts <- Biostrings::oligonucleotideFrequency(Biostrings::DNAStringSet(sequences), k)
    homopolymerCorrection(sequences, k, colSums(kmer.counts))
  }, error = function(e) {
    kmer.counts <- Biostrings::oligonucleotideFrequency(Biostrings::RNAStringSet(sequences), k)
    rna.kmers <- homopolymerCorrection(sequences, k, colSums(kmer.counts), is.rna = TRUE)
...
#'
#' @description
#' Compares foreground sequence set to background sequence set and computes enrichment values
...
#' (generated by \code{\link{generateKmers}})
#' @param permutation if \code{TRUE}, only the enrichment value is returned (efficiency mode
#' used for permutation testing)
#' @param chisq.p.value.threshold threshold below which Fisher's exact test is used instead of
#' \eqn{\Chi^2} test
...
#'   \code{p.value} \tab p-value of \emph{k}-mer enrichment (either from Fisher's exact test or
#'   \eqn{\Chi^2} test.\cr
#'   \code{adj.p.value} \tab multiple testing corrected p-value\cr
#' }
#'
#' @details
#' Usually uses Pearson's \eqn{\Chi^2} test, but recalculates p-values with Fisher's exact test
#' for \eqn{\Chi^2} test p-values \code{<= chisq.p.value.threshold}. The reason this is done is
#' computational efficiency. Fisher's exact tests are computationally demanding and are only
...
#' @family \emph{k}-mer functions
#' @export
computeKmerEnrichment <- function(foreground.kmers, background.kmers, permutation = FALSE,
                                  chisq.p.value.threshold = 0.05, p.adjust.method = "BH") {
  background.kmers.sum <- sum(background.kmers)
  other.background.kmers <- background.kmers.sum - background.kmers

  foreground.kmers.sum <- sum(foreground.kmers)
  other.foreground.kmers <- foreground.kmers.sum - foreground.kmers

  # calculate enrichment value
  if (foreground.kmers.sum == 0 || background.kmers.sum == 0) {
      enrichment <- as.numeric(NA)
  } else {
      enrichment <- (foreground.kmers / foreground.kmers.sum) / (background.kmers / background.kmers.sum)
  }
  ...
  if(permutation) {
    ...
    # calculate Fisher's exact test for Chi-squared test p-values <= chisq.p.value.threshold
    idx <- which(stats::p.adjust(chisq.p.values, method = p.adjust.method) <= chisq.p.value.threshold)
    ...
    if(length(idx) > 0) {
      ...
      fisher.p.values <- vapply(idx, function(i) {
        cont <- matrix(c(foreground.kmers[i],
                         ...
                         background.kmers[i],
                         other.background.kmers[i]), nrow = 2)
        return(stats::fisher.test(cont)$p.value)
      })

      # replace Chi-squared test p-values with Fisher's exact test p-values
      chisq.p.values[idx] <- fisher.p.values
    }

    # adjust for multiple testing
    adj.p.value <- stats::p.adjust(chisq.p.values, method = p.adjust.method)
    df <- data.frame(foreground.count = foreground.kmers,
...
#'
#' @description
#' \code{empiricalEnrichmentMeanCDF} returns an estimate of the significance of the observed
#' mean, given a vector of means based on random permutations of the data.
#'
#' @param random.means numeric vector of means based on random permutations of the data
#' (empirical null distribution)
#' @param actual.mean observed mean
#' @param alternative side of the test, one of the following: \code{"two.sided"},
...
#' @family \emph{k}-mer functions
#' @export
empiricalEnrichmentMeanCDF <- function(random.means, actual.mean,
...
#'
#' @param x numeric vector of values for which the geometric mean will be computed
...
#'
#' @description
#' Calculates \emph{k}-mer enrichment values for randomly sampled (without replacement) foreground sets.
#'
#' @param n.transcripts.foreground number of transcripts in the original foreground set
#' @param n.permutations number of permutations to perform
#' @inheritParams calculateKmerEnrichment
#'
#' @return The result of \code{\link{calculateKmerEnrichment}} for the random foreground sets.
#'
#' @family \emph{k}-mer functions
#' @export
generatePermutedEnrichments <- function(n.transcripts.foreground, background.set, k,
  ...
  return(calculateKmerEnrichment(random.foreground.sets, background.set, k,
                                 permutation = TRUE, n.cores = n.cores))
}

#' @title Permutation Test Based Significance of Observed Mean
#'
#' @description
#' \code{permTestGeometricMean} returns an estimate of the significance of the observed
#' mean, given a set of random permutations of the data.
#'
#'
#' @param motif.kmers set of \emph{k}-mers that were used to compute the \code{actual.mean}
#' @param random.permutations a set of random permutations of the original data, used
#' to generate an empirical null distribution.
#' @param produce.plot if distribution plot should be part of the returned list
...
#'   \code{plot} \tab plot of the empirical distribution of geometric means of the
#'   enrichment values
#' }
#'
...
#' @family \emph{k}-mer functions
#' @export
permTestGeometricMean <- function(actual.mean, motif.kmers, random.permutations,
                                  alternative = c("two.sided", "less", "greater"),
                                  conf.level = 0.95, produce.plot = TRUE) {
  # avoid CRAN note
  means <- actual <- ..density.. <- NULL

  idx <- which(random.permutations$kmers %in% motif.kmers)
  random.means <- unlist(lapply(random.permutations$dfs, function(enrichment.values) {
    return(geometricMean(enrichment.values[idx]))
  }))

  if(produce.plot) {
    df <- data.frame(means = random.means, actual = rep(actual.mean, length(random.means)))
    bin.width <- (max(df$means) - min(df$means)) / 40
    if(bin.width <= 0) {
      bin.width <- 0.001
    }
    dist.plot <- ggplot2::ggplot(df, ggplot2::aes(x = means)) +
      ggplot2::geom_histogram(ggplot2::aes(y = ..density..),
                              colour = "black", fill = "white", binwidth = bin.width) +
      ggplot2::geom_density(alpha = .2, fill = "#66FF66") +
      ggplot2::geom_vline(ggplot2::aes(xintercept = mean(actual)),
      ...
      ggplot2::ggtitle("Monte Carlo sampling of distribution of mean enrichment values")
  } else {
    dist.plot <- NULL
  }

  empirical.p.value <- empiricalEnrichmentMeanCDF(random.means, actual.mean,
                                                  alternative = alternative, conf.level)
  return(list(p.value.estimate = empirical.p.value$p.value.estimate,
...
#'
#' @description
#' Uses a volcano plot to visualize \emph{k}-mer enrichment. X-axis is \eqn{\log_2} enrichment value,
#' y-axis is \eqn{\log_10} significance, i.e., multiple testing corrected p-value from
#' Fisher's exact test or \eqn{\Chi^2} test.
#'
#' @param kmers data frame with the following columns: kmer, adj.p.value, enrichment
#' @param motif.kmers set of \emph{k}-mers that are associated with a certain motif,
#' will be highlighted in volcano plot
#' @param motif.rbps name of RNA-binding proteins associated with highlighted \emph{k}-mers
#' (character vector of length 1)
#' @param significance.threshold p-value threshold for significance, e.g., \code{0.05} or
#' \code{0.01}
...
#' @family \emph{k}-mer functions
#' @export
drawVolcanoPlot <- function(kmers, motif.kmers, motif.rbps, significance.threshold = 0.01,
                            show.legend = TRUE) {
  # avoid CRAN note
  log.enrichment <- log.adj.p.value <- group <- motifidx <- NULL

  indices <- as.character(kmers$kmer) %in% motif.kmers

  kmers$sig <- kmers$adj.p.value < significance.threshold
  kmers$log.adj.p.value <- -log10(as.numeric(kmers$adj.p.value))
  kmers$enrichment[is.na(kmers$enrichment)] <- 1
  kmers$log.enrichment <- log2(as.numeric(kmers$enrichment))
  kmers$group[kmers$adj.p.value >= significance.threshold] <- "n"
  kmers$group[kmers$enrichment > 1 & kmers$adj.p.value <= significance.threshold] <- "e"
  kmers$group[kmers$enrichment < 1 & kmers$adj.p.value <= significance.threshold] <- "d"
  kmers$group[indices == TRUE] <- "m"
  kmers$motifidx <- indices
  part1 <- kmers[which(indices == TRUE), ]
  part2 <- kmers[which(kmers$adj.p.value < significance.threshold), ]

  volcano.plot <- ggplot2::ggplot(kmers, ggplot2::aes(log.enrichment,
                                                      log.adj.p.value,
                                                      color = group, alpha = group,
                                                      size = motifidx)) +
    ggplot2::geom_point() +
    ggplot2::scale_color_manual(values = c("n" = "#000000", "m" = "yellow",
                                           "e" = "#832424FF", "d" = "#004F91FF"),
                                name = "", breaks = c("d", "e", "m", "n"),
                                labels = c("depleted", "enriched",
                                           paste0(motif.rbps, " motif kmer"), "not significant")) +
    ggplot2::scale_size_manual(values = c(4, 6), guide = FALSE) +
    ggplot2::scale_alpha_manual(values = c(0.5, 0.5, 0.7, 0.5), guide = FALSE) +
    ...
    ggplot2::geom_point(data = dplyr::filter(kmers, group == "m"),
                        ggplot2::aes(log.enrichment, log.adj.p.value), shape = 21,
                        color = "#000000", fill = "yellow") +
    ggplot2::theme_bw(base_size = 16) +
    ggplot2::theme(axis.line = ggplot2::element_line(colour = "black"),
                   panel.grid.major = ggplot2::element_blank(),
                   panel.grid.minor = ggplot2::element_blank(),
                   ...
                   legend.position = "top")
  if (!show.legend) {
    volcano.plot <- volcano.plot + ggplot2::theme(legend.position = "none")
  }
...
#'
#' @description
#' Checks if the provided set of \emph{k}-mers is valid. A valid set of \emph{k}-mers is
#' (1) non-empty, (2) contains either only hexamers or only heptamers, and (3) contains
...
checkKmers <- function(kmers) {
  ...
  invalid.characters <- gsub("A", "", gsub("C", "", gsub("G", "", gsub("U", "", kmers))))
