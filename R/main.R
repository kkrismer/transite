...
#'
#' @param foreground.sets a list of named character vectors of
    ...
    function(background.set,
        ...
        if (length(results$enrichment.dfs) > 0) {
            ...
            spectrum.info <- lapply(motifs, function(motif) {
                ...
                return(
                    list(
                        info = list(
                            motif.id = motifId(motif),
                            motif.rbps = paste(motifRbps(motif), collapse = ", "),
            ...
            spectrum.info.df$f.statistic.adj.p.value <-
                stats::p.adjust(spectrum.info.df$f.statistic.p.value,
                                method = p.adjust.method)
            spectrum.info.df$consistency.score.adj.p.value <-
                stats::p.adjust(spectrum.info.df$consistency.score.p.value,
                                method = p.adjust.method)
...
#' @param kmer.significance.threshold p-value threshold for significance,
#' e.g., \code{0.05} or
#' \code{0.01} (used for volcano plots)
#' @param produce.plot if \code{TRUE} volcano plots and distribution plots
#' are created
#' @param p.combining.method one of the following: Fisher (1932)
                ...
                lapply(seq_len(length(foreground.sets)), function(i) {
                    ...
                    foreground.result <-
                        lapply(motifs, function(motif) {
                            if (k == 6) {
                                rbp.kmers <- motifHexamers(motif)
                            } else if (k == 7) {
                                rbp.kmers <- motifHeptamers(motif)
                            }
                            ...
                            return(
                                list(
                                    df = list(
                                        motif.id = motifId(motif),
                                        motif.rbps = paste0(motifRbps(motif),
                                                            collapse = ", "),
                    ...
                    df$adj.p.value.estimate <-
                        stats::p.adjust(df$p.value.estimate,
                                        method = p.adjust.method)
                    df$adj.enriched.kmers.combined.p.value <-
                        stats::p.adjust(df$enriched.kmers.combined.p.value,
                                        method = p.adjust.method)
                    df$adj.depleted.kmers.combined.p.value <-
                        stats::p.adjust(df$depleted.kmers.combined.p.value,
                                        method = p.adjust.method)
