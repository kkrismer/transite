...
#' foreground.set1 <- c("CAACAGCCTTAATT", "CAGTCAAGACTCC", "CTTTGGGGAAT",
...
#' names(foreground.set1) <- c("NM_1_DUMMY|3UTR", "NM_2_DUMMY|3UTR", "NM_3_DUMMY|3UTR",
...
#' names(foreground.set2) <- c("NM_15_DUMMY|3UTR", "NM_16_DUMMY|3UTR", "NM_17_DUMMY|3UTR",
#'   "NM_18_DUMMY|3UTR")
#'
#' foreground.sets <- list(foreground.set1, foreground.set2)
#'
#' background.set <- c("CAACAGCCTTAATT", "CAGTCAAGACTCC", "CTTTGGGGAAT",
#'                                    "TCATTTTATTAAA", "AATTGGTGTCTGGATACTTCCCTGTACAT",
#'                                    "ATCAAATTA", "AGAT", "GACACTTAAAGATCCT",
#'                                    "TAGCATTAACTTAATG", "ATGGA", "GAAGAGTGCTCA",
#'                                    "ATAGAC", "AGTTC", "CCAGTAA",
...
#' names(background.set) <- c("NM_1_DUMMY|3UTR", "NM_2_DUMMY|3UTR", "NM_3_DUMMY|3UTR",
#'   "NM_4_DUMMY|3UTR", "NM_5_DUMMY|3UTR", "NM_6_DUMMY|3UTR", "NM_7_DUMMY|3UTR",
#'   "NM_8_DUMMY|3UTR", "NM_9_DUMMY|3UTR", "NM_10_DUMMY|3UTR", "NM_11_DUMMY|3UTR",
...
#' toy.motif <- createMatrixMotif("toy.motif", "example RBP", toy.motif.matrix,
...
#' @export
runMatrixTSMA <- function(foreground.sets, background.set, motifs = NULL, max.hits = 5,
                          threshold.method = "p.value", threshold.value = 0.25^6,
                          max.fg.permutations = 1000000, min.fg.permutations = 1000, e = 5,
                          p.adjust.method = "BH", n.cores = 1, cache = paste0(getwd(), "/sc/")) {
  # avoid CRAN note
  adj.p.value <- p.value <- NULL

  i <- 1
  foreground.scores <- lapply(foreground.sets, function(foreground.set) {
    result <- scoreTranscripts(foreground.set, motifs = motifs, max.hits = max.hits,
                               threshold.method = threshold.method, threshold.value = threshold.value,
                               n.cores = n.cores, cache = cache)
    message(paste0("scored transcripts in foreground set ", i))
    i <<- i + 1
    return(result)
  })

  background.scores <- scoreTranscripts(background.set, motifs = motifs, max.hits = max.hits,
  ...
  i <- 1
  enrichment.dfs <- lapply(foreground.scores, function(scores.per.condition) {
    enrichment.df <- calculateMotifEnrichment(scores.per.condition$df, background.scores$df,
                                         ...
                                         min.fg.permutations = min.fg.permutations, e = e,
                                         p.adjust.method = p.adjust.method)
    enrichment.df <- dplyr::arrange(enrichment.df, adj.p.value, p.value)
    message(paste0("calculated enrichment for foreground set ", i))
    i <<- i + 1
    return(enrichment.df)
  })
  return(list(foreground.scores = foreground.scores, background.scores = background.scores,
...
#' @export
runMatrixSPMA <- function(background.set, motifs = NULL, n.bins = 40, max.model.degree = 1,
                          max.cs.permutations = 10000000, min.cs.permutations = 5000, max.hits = 5,
                          threshold.method = "p.value", threshold.value = 0.25^6,
                          max.fg.permutations = 1000000, min.fg.permutations = 1000, e = 5,
                          p.adjust.method = "BH", n.cores = 1, cache = paste0(getwd(), "/sc/")) {
  # avoid CRAN note
  motif.id <- motif.rbps <- adj.r.squared <- degree <- residuals <- slope <- NULL
  f.statistic <- f.statistic.p.value <- f.statistic.adj.p.value <- NULL
  consistency.score <- consistency.score.p.value <- consistency.score.adj.p.value <- consistency.score.n <- NULL
  n.significant <- n.very.significant <- n.extremely.significant <- aggregate.classifier.score <- NULL
  ...
  if(length(results$enrichment.dfs) > 0) {
    ...
    spectrum.info <- lapply(motifs, function(motif) {
      ...
      values[values == 0] <- 0.01 # avoid -Inf after taking the log
      score <- scoreSpectrum(log(values), motif.data.df$adj.p.value,
                             max.model.degree = max.model.degree,
                             max.cs.permutations = max.cs.permutations,
                             min.cs.permutations = min.cs.permutations)

      n.significant <- sum(motif.data.df$adj.p.value <= 0.05)
      classifier.score <- spectrumClassifier(score$adj.r.squared, score$degree, score$slope,
                                             score$consistency.score.n,
                                             n.significant, n.bins)

      return(list(info = list(motif.id = motif$id,
    ...
    spectrum.info.df <- as.data.frame(do.call("rbind", lapply(spectrum.info, function(x) x$info)),
                                      stringsAsFactors = FALSE)
    spectrum.info.df$motif.id <- as.character(spectrum.info.df$motif.id)
    spectrum.info.df$motif.rbps <- as.character(spectrum.info.df$motif.rbps)
    spectrum.info.df$adj.r.squared <- as.numeric(spectrum.info.df$adj.r.squared)
    spectrum.info.df$degree <- as.integer(spectrum.info.df$degree)
    spectrum.info.df$residuals <- as.numeric(spectrum.info.df$residuals)
    spectrum.info.df$slope <- as.numeric(spectrum.info.df$slope)
    spectrum.info.df$f.statistic <- as.numeric(spectrum.info.df$f.statistic)
    spectrum.info.df$f.statistic.p.value <- as.numeric(spectrum.info.df$f.statistic.p.value)
    spectrum.info.df$consistency.score <- as.numeric(spectrum.info.df$consistency.score)
    spectrum.info.df$consistency.score.p.value <- as.numeric(spectrum.info.df$consistency.score.p.value)
    spectrum.info.df$consistency.score.n <- as.integer(spectrum.info.df$consistency.score.n)
    spectrum.info.df$n.significant <- as.integer(spectrum.info.df$n.significant)
    spectrum.info.df$n.very.significant <- as.integer(spectrum.info.df$n.very.significant)
    spectrum.info.df$n.extremely.significant <- as.integer(spectrum.info.df$n.extremely.significant)
    spectrum.info.df$aggregate.classifier.score <- as.integer(spectrum.info.df$aggregate.classifier.score)

    spectrum.info.df$f.statistic.adj.p.value <- stats::p.adjust(spectrum.info.df$f.statistic.p.value, method = p.adjust.method)
    spectrum.info.df$consistency.score.adj.p.value <- stats::p.adjust(spectrum.info.df$consistency.score.p.value, method = p.adjust.method)

    spectrum.info.df <- dplyr::select(spectrum.info.df, motif.id, motif.rbps, adj.r.squared, degree, residuals, slope,
                                      f.statistic, f.statistic.p.value, f.statistic.adj.p.value,
                                      consistency.score, consistency.score.p.value, consistency.score.adj.p.value, consistency.score.n,
                                      n.significant, n.very.significant, n.extremely.significant, aggregate.classifier.score)

    spectrum.plots <- lapply(spectrum.info, function(x) x$spectrum.plot)
    classifier.scores <- lapply(spectrum.info, function(x) x$classifier.score)
  } else {
    spectrum.info.df <- data.frame(motif.id = character(0),
  ...
  return(list(foreground.scores = results$foreground.scores, background.scores = results$background.scores, enrichment.dfs = results$enrichment.dfs,
              spectrum.info.df = spectrum.info.df, spectrum.plots = spectrum.plots, classifier.scores = classifier.scores))
}
...
#'
#' # run TSMA with user-defined motif
#' toy.motif <- createKmerMotif("toy.motif", "example RBP",
...
#' @importFrom dplyr filter
#' @export
runKmerTSMA <- function(foreground.sets, background.set, motifs = NULL, k = 6,
  ...
  enrichment.result <- calculateKmerEnrichment(foreground.sets, background.set, k,
                                               p.adjust.method = p.adjust.method, n.cores = n.cores)
  ...
  if(!is.null(enrichment.result)) {
    ...
    for(set.size in set.sizes) {
      ...
      assign(as.character(set.size),
             generatePermutedEnrichments(set.size,
                                         background.set,
                                         k,
    ...
    motif.result <- lapply(seq_len(length(foreground.sets)), function(i) {
      ...
      foreground.result <- lapply(motifs, function(motif) {
        if(k == 6) {
        ...
        idx <- which(enrichment.result$kmers %in% rbp.kmers)
        motif.kmers.df <- kmers.df[idx, ]
        motif.kmers.df <- dplyr::select(motif.kmers.df, kmer, enrichment, p.value, adj.p.value)

        geo.mean <- geometricMean(motif.kmers.df$enrichment)
        perm.test <- permTestGeometricMean(geo.mean, rbp.kmers,
                                           get(as.character(length(foreground.sets[[i]])),
                                               envir = random.enrichments,
                                               inherits = FALSE),
                                           alternative = "two.sided", conf.level = 0.95, produce.plot = produce.plot)

        motif.kmers.enriched.df <- dplyr::filter(motif.kmers.df, enrichment > 1)
        enriched.kmers.combined.p.value <- pCombine(motif.kmers.enriched.df$adj.p.value, method = p.combining.method)

        motif.kmers.depleted.df <- dplyr::filter(motif.kmers.df, enrichment < 1)
        depleted.kmers.combined.p.value <- pCombine(motif.kmers.depleted.df$adj.p.value, method = p.combining.method)

        if(produce.plot) {
          volcano.plot <- drawVolcanoPlot(kmers.df, rbp.kmers, paste(motif$rbps, collapse = ", "), kmer.significance.threshold)
        } else {
          volcano.plot <- NULL
        }
      ...
      df <- as.data.frame(do.call("rbind", lapply(foreground.result, function(x) x$df)), stringsAsFactors = FALSE)
      df$motif.id <- as.character(df$motif.id)
      df$motif.rbps <- as.character(df$motif.rbps)
      df$geo.mean.enrichment <- as.numeric(df$geo.mean.enrichment)
      df$p.value.estimate <- as.numeric(df$p.value.estimate)
      df$cont.int.low <- as.numeric(df$cont.int.low)
      df$cont.int.high <- as.numeric(df$cont.int.high)
      df$enriched.kmers.combined.p.value <- as.numeric(df$enriched.kmers.combined.p.value)
      df$depleted.kmers.combined.p.value <- as.numeric(df$depleted.kmers.combined.p.value)

      df$adj.p.value.estimate <- stats::p.adjust(df$p.value.estimate, method = p.adjust.method)
      df$adj.enriched.kmers.combined.p.value <- stats::p.adjust(df$enriched.kmers.combined.p.value, method = p.adjust.method)
      df$adj.depleted.kmers.combined.p.value <- stats::p.adjust(df$depleted.kmers.combined.p.value, method = p.adjust.method)

      motif.kmers.dfs <- lapply(foreground.result, function(x) x$motif.kmers.df)
      volcano.plots <- lapply(foreground.result, function(x) x$volcano.plot)
      perm.test.plots <- lapply(foreground.result, function(x) x$perm.test.plot)
      enriched.kmers.combined.p.values <- lapply(foreground.result, function(x) x$enriched.kmers.combined.p.value)
      depleted.kmers.combined.p.values <- lapply(foreground.result, function(x) x$depleted.kmers.combined.p.value)

      return(list(df = df, motif.kmers.dfs = motif.kmers.dfs, volcano.plots = volcano.plots, perm.test.plots = perm.test.plots,
...
#' @importFrom dplyr filter
#' @export
runKmerSPMA <- function(background.set, motifs = NULL, k = 6, n.bins = 40, max.model.degree = 1,
                        max.cs.permutations = 10000000, min.cs.permutations = 5000,
                        fg.permutations = 5000,
                        p.adjust.method = "BH", p.combining.method = "fisher", n.cores = 1) {
  # avoid CRAN note
  motif.id <- motif.rbps <- adj.r.squared <- degree <- residuals <- slope <- NULL
  f.statistic <- f.statistic.p.value <- f.statistic.adj.p.value <- NULL
  consistency.score <- consistency.score.p.value <- consistency.score.adj.p.value <- NULL
  consistency.score.n <- aggregate.classifier.score <- NULL
  n.significant <- n.very.significant <- n.extremely.significant <- NULL
  ...
  if(length(results) > 0) {
    dfs <- lapply(results, function(result) { result$motif.df} )
    enrichment.df <- do.call("rbind", dfs)
    enrichment.df$adj.p.value.estimate <- stats::p.adjust(enrichment.df$p.value.estimate,
    ...
    spectrum.info <- lapply(motifs, function(motif) {
      ...
      values[values == 0] <- 0.01 # avoid -Inf after taking the log
      score <- scoreSpectrum(log(values), motif.data.df$adj.p.value.estimate,
                             max.model.degree = max.model.degree,
                             max.cs.permutations = max.cs.permutations,
                             min.cs.permutations = min.cs.permutations)

      n.significant <- sum(motif.data.df$adj.p.value.estimate <= 0.05)
      classifier.score <- spectrumClassifier(score$adj.r.squared, score$degree, score$slope,
                                             score$consistency.score.n,
                                             n.significant, n.bins)
    ...
    spectrum.info.df <- dplyr::select(spectrum.info.df, motif.id, motif.rbps, adj.r.squared, degree,
                                      residuals, slope,
                                      f.statistic, f.statistic.p.value, f.statistic.adj.p.value,
                                      consistency.score, consistency.score.p.value,
                                      consistency.score.adj.p.value, consistency.score.n,
                                      n.significant, n.very.significant, n.extremely.significant,
  ...
  return(list(foreground.scores = results,
              spectrum.info.df = spectrum.info.df, spectrum.plots = spectrum.plots,
